...
async def link_handler(Mbot, message):
    ...
    try:
        ...
        if url.endswith("="):
           dump_file=await message.reply_video(url[:-1],caption="Downloaded By @UncutFlixBot")
        else:
            dump_file=await message.reply_video(url,caption="Downloaded By @UncutFlixBot")
        ...
        try:
            if "/reel/" in url:
               ...
               try:
                   if ddinsta:
                      dump_file=await message.reply_video(content_value,caption="Downloaded By @UncutFlixBot")
                   else:
                       dump_file=await message.reply_video(content_value, caption="Downloaded By @UncutFlixBot")
               except:
              ...
              #    await message.reply(meta)
                  for i in range(len(meta) - 1):
                     ...
                     try:
                        dump_file=await message.reply_video(com.text,caption="Downloaded By @UncutFlixBot")
                        await com.delete()
                     except:
                         pass 
            elif "stories" in url:
